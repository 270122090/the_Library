    ...
    , ui(new Ui::MainWindow)
{
    ...
    //set the due date
        ...
        display_overduebooks();
        ui->listoBooks->setCurrentRow(0);

        connect(ui->LoadoBookList, &QAction::triggered,this, &MainWindow::display_overduebooks);
        connect(ui->btnReminder, &QPushButton::clicked, this, &MainWindow::overdue_BookQINFO);
...
void MainWindow::add_a_member() // * working
{
        ...
        if (NewMember != nullptr)
        {
            ...
            ui->listiMember->addItem("     " + NewMember->getMemberName()+"     |     " + NewMember->getPhoneNumber());// space left delibrately
        }
...
void MainWindow::display_membersdetails()// * working
{
...

    {
        AllMembers* currentmember = memberList.at(index);
            ...
            //ui->lblMemberFine->setText(currentmember->getFine());
...
void MainWindow::add_a_Book() // * Working
{
    ...
    if (newBook != nullptr)
    {
        ...
        ui->listiBooks->addItem("     "+newBook->getid()+"     " +newBook->gettitle()+"      " +newBook->getauthor());
    }
...
void MainWindow::update_a_Book()// * working
{
...
void MainWindow::display_booklist()// * working
{
    ...
    while(!in.atEnd())
    {
        ...
        ui->listiBooks->addItem("     "+bookinfo.at(4)+"      " +bookinfo.at(0)+"      " +bookinfo.at(1));

                                            // title        author           dewey          status          id              dd              image
        AllBooks* bookproduct = new AllBooks(bookinfo.at(0), bookinfo.at(1),bookinfo.at(2), bookinfo.at(3), bookinfo.at(4), bookinfo.at(5), bookinfo.at(6));
        booklist.push_back(bookproduct);
    }
    in.flush();
...
void MainWindow::return_a_Book()// *working
{
    int index = ui->listBooks->currentRow();
...
//    if(book1=="")
//    {
//        issuemember->setBook1(bookissued);}
//    else{
//        if(book2=="")
//        {
//         issuemember->setBook2(bookissued);}
//        else
//        {
//            issuemember->setBook3(bookissued);}
...
void MainWindow::search_i_member() // * working
{
...
void MainWindow::display_bookidetails() // * workng
{
...
void MainWindow::search_i_Book()//* working
{
...
void MainWindow::issue_a_Book()//
{
            ...
            AllMembers* issuemember=memberList.at(memberindex);
            QString bookissued = ui->iBookTitle->text();
            QString book1 = ui->lblMemberBook1->text();
            QString book2 = ui->lblMemberBook2->text();
            QString book3 = ui->lblMemberBook3->text();

            if(currentstatus == "On Shelf")
                {

                    ui->inewStatus->setText("On Loan");

                    issuebook->setdd(dd.toString());
                    issuebook->setstatus(issuestatus);

                    if(book1=="")
                        {
                            issuemember->setBook1(bookissued);
                            ui->lblMemberBook1->setText(issuemember->getBook1());
                        }
                    else if (book2=="")
                        {
                            issuemember->setBook2(bookissued);
                            ui->lblMemberBook2->setText(issuemember->getBook2());
                        }
                    else if (book3=="")
                        {
                        issuemember->setBook3(bookissued);
                        ui->lblMemberBook3->setText(issuemember->getBook3());
                        }
                    else
                        {
                        QMessageBox::warning(this,"ISSUE ERROR", "Member exceeded number of copies permitted");
                        }
            ...
            QFile bookupdateFile("books.txt");
            bookupdateFile.open(QIODevice::ReadOnly | QIODevice::Text);

            QTextStream in(&bookupdateFile);
            ...
            if (issuebook!=nullptr)
            {
                QFile bookFile("books.txt");
                bookFile.open(QIODevice::WriteOnly| QIODevice::Text);
                QTextStream outb(&bookFile);
                for(int i=0;i<booklist.size();i++)
                   {
                    outb << booklist.at(i)->gettitle()<<",";
                    outb << booklist.at(i)->getauthor()<<",";
                    outb << booklist.at(i)->getdewey()<<",";
                    outb << booklist.at(i)->getstatus()<<",";
                    outb << booklist.at(i)->getid()<<",";
                    outb << booklist.at(i)->getdd()<<",";
                    outb << booklist.at(i)->getimage()<< Qt::endl;
                    }
                outb.flush();
                bookFile.close();
            }
                ...
                //****** MEMBER ***********
                    ...
                    //                QFile memberFile("members.txt");
                    //                memberFile.open(QIODevice::WriteOnly|QIODevice::Text);
                    //                QTextStream outm(&memberFile);

                    //                for(int i=0;i<memberList.size();i++)
                    //                    {
                    //                                outm << memberList.at(i)->getMemberName()<<",";
                    //                                outm << memberList.at(i)->getPhoneNumber()<<",";
                    //                                outm << memberList.at(i)->getEmail()<<",";
                    //                                outm << memberList.at(i)->getBook1()<<",";
                    //                                outm << memberList.at(i)->getBook2()<<",";
                    //                                outm << memberList.at(i)->getBook3()<< Qt::endl;
...
//                QFile memberupdateFile("members.txt");
//                memberupdateFile.open(QIODevice::ReadOnly | QIODevice::Text);
//                QTextStream inm(&memberupdateFile);
...
void MainWindow::overdue_BookQINFO()
{
...
void MainWindow::display_overduebooks()
{
